...
#### 重排&重汇
重排（回流|reflow）：布局改变，重新排列元素
重汇： 某些元素的外观被改变，例如：元素的填充颜色  

注意：
* 重绘不一定导致重排，但重排一定会导致重绘
* 浏览器的渲染队列机制：当修改元素的几何属性，导致浏览器触发重排或重绘时，它会把该操作放进渲染队列，等到队列中的操作到了一定的数量或者到了一定的时间间隔时，浏览器就会批量执行这些操作。

优化：
* 重排：
    * 减少重排范围：
        1. 改变样式尽可能在低层级的DOM节点上
        2. 不要使用table布局，用的话，可以用table-layout:auto、table-layout:fixed，让table一行一行的渲染
    * 减少重排次数：
        1. 样式集中改变：更改类名 > el.style.cssText > el.style
        2. 分离读写操作：DOM 的多个读操作（或多个写操作），应该放在一起
        3. 将 DOM 离线：不在当前的DOM树中做修改：
            * 先用 display:none，完成修改后，再显示
            * 通过 documentFragment 创建一个 dom 碎片,在它上面批量操作 dom后再append到文档中
            * 复制节点，在副本上工作，然后替换
        4. 使用 absolute 或 fixed 脱离文档流
        5. 优化动画：
            * 把动画效果应用到 position属性为 absolute 或 fixed 的元素上
            * GPU 加速，包括Canvas2D，布局合成, CSS3转换（transitions），CSS3 3D变换（transforms），WebGL和视频(video)
